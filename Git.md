...
​	5. `git add readme.txt`		//告诉git添加一个文档

​	6. `git commit -m "wrote a readme file"	`	//提交    -m 是添加提交说明

​	7. `git log`  	//查看之前的操作	--pretty=oneline  单行显示，方便查看

​	8. `git reset --hard HEAD^`		//退回往上的一个版本，HEAD~100就是往上一个版本

​	9. `git reset --hard 版本号`		//只需要知道版本的前几位数字

​	10. `git  reflog`		//查看每次命令

​	11. `git status`		//查看暂存区的所有文件

​	12. `git diff HEAD -- readme.txt`		//查看工作区和版本库里最新版本区别

​	13. `git check -- readme.txt`		//将在提交前使用，r如果已提交则无法还原，eadme.txt文件在工作区的修改全部撤销掉，回到add或commit

​	14. `git status`		//查看暂存区的在状态

​	15. `git rm readme.txt` 	//删除工作区里的文件同时也要删除暂存区的文件

​	16. `git commit -m "delete"	`	//提交删除命令

​	17. `git remote add origin https://github.com/kssdesummer/learngit`		//(远程库网址)连接远程库，origin是指远程库的名字，可以另外设置，如果连接的远程库较多，则需要区别

​	18. `git push -u origin master	`	//将本地库的所有信息推送到远程库上，输入账号密码

​	19. `git remote -v	`	//查看项目远程地址

​	20. `git branch -a`		//查看当前所在分支（红色），以及其他分支

​	21. `git  remote show origin`		//查看当前仓库其本信息

​	22. `git remote rm origin`		//删除远程库origin

​	23. `git checkout -b dev`		//创建并切换到dev分支 ：`git branch dev` ;  `dit checkout dev`

​	24. `git branch	`	//查看当前分支，代*为当前分支

​	25. `git checkout master`		//切换会master分支

​	26. `git merge dev`		//将指定分支合并到master上

​	27. `git branch -d dev`		//删除指定分区

​	28. `git merge --no-ff -m "说明" 分支名`		//合并分支时禁止Fast forward模式，就是合并不删除原分支的历史纪录，可以看到做个合并，创建新的commit

​	29. `git branch -D 分支名`		//删除未合并的分支
